...
- 想快捷管理录制链接时，可在`config.ini`中配置`tgapi令牌`、`tg聊天id(个人或者群组id)`并开启`tg快捷管理录制地址(是/否)=是`，然后给Bot发送命令：`/add 直播间链接`（新增）、`/list`（查询）、`/del 直播间链接`（删除）、`/update 旧链接|新链接`（修改）；也支持直接发送直播间链接自动新增到`URL_config.ini`。
